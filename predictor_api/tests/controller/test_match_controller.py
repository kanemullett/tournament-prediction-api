...
class TestMatchController:
    ...
    async def test_should_pass_updated_matches_as_response(self):
        ...
        self.__service.update_matches.return_value = [
            Match(
                id=UUID("8efaf853-980e-4607-9b45-d854460ec5e0"),
                homeTeam=Team(
                    id=UUID("bbec1707-7ea3-49cb-9791-7a1358a2b894"),
        ...
        # When
        matches: list[Match] = await self.__controller.update_matches(
            UUID("5341cff8-df9f-4068-8a42-4b4288ecba87"),
            matches
        )
    ...
    async def test_should_pass_error_update_matches(self):
        ...
        self.__service.update_matches.side_effect = HTTPException(
            status_code=404,
            detail="Not Found"
        )
        ...
        # When
        with raises(HTTPException) as httpe:
            await self.__controller.update_matches(
                UUID("1c29f6c3-9f98-41a8-ba0f-e07013742797"),
                matches
            )
