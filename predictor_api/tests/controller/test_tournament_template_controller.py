...
class TestTournamentTemplateController:
    ...
    __controller: TournamentTemplateController = TournamentTemplateController(
        __service
    )
    ...
    async def test_should_pass_tournament_templates_as_response(self):
        # Given
        self.__service.get_tournament_templates.return_value = [
            TournamentTemplate(
        ...
        tournament_templates: list[KnockoutTemplate] = \
            await self.__controller.get_tournament_templates()
    ...
    async def test_should_pass_created_tournament_templates_as_response(self):
        ...
        self.__service.create_tournament_templates.return_value = [
            TournamentTemplate(
                id="c08fd796-7fea-40d9-9a0a-cb3a49cce2e4",
                name="tournament1",
                knockout=KnockoutTemplate(
        ...
        # When
        created: list[TournamentTemplate] = await (
            self.__controller.create_tournament_templates(
    ...
    async def test_should_pass_found_tournament_template_as_response(self):
        # Given
        self.__service.get_tournament_template_by_id.return_value = (
            TournamentTemplate(
                id="d15956ef-d199-40b1-b7d7-850a9add97e7",
                name="tournament3",
                league=LeagueTemplate(
        ...
        tournament_template: TournamentTemplate = \
            await self.__controller.get_tournament_template_by_id(
                UUID("d15956ef-d199-40b1-b7d7-850a9add97e7")
            )
    ...
    async def test_should_pass_error_if_tournament_template_not_found(self):
        # Given
        self.__service.get_tournament_template_by_id.side_effect = (
            HTTPException(
                status_code=404,
    ...
    @pytest.mark.asyncio
    async def test_error_if_tournament_template_is_being_used(self):
        # Given
        self.__service.delete_tournament_template_by_id.side_effect = (
            HTTPException(
                status_code=409,
        ...
        # When
        with raises(HTTPException) as httpe:
            await self.__controller.delete_tournament_template_by_id(
                UUID("c08fd796-7fea-40d9-9a0a-cb3a49cce2e4")
            )
